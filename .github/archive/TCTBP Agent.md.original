...
### 5a. Release Notes (Optional)

This repo does not use a `CHANGELOG.md`. Instead, user-facing release notes are maintained in `releaseNotes.ts` (see `spfx/intranet-core/src/webparts/intranetShell/components/data/releaseNotes.ts`) and displayed in the app's "What's New" panel.

- Use `scripts/add-release-note.ps1 -Title "..." -Category feature|improvement|bugfix|security` to add placeholder entries.
- Update the placeholder with specific changes before shipping.
- Release notes are grouped by minor version (v0.5, v0.6, etc.).

If the changeset is **docs-only or infrastructure-only**, skip this step.
...
```json
{
  ...
  "project": {
    "defaultBranch": "main",
    "packageManager": "npm",
    "versionFiles": ["spfx/intranet-core/package.json", "spfx/intranet-core/config/package-solution.json"],
    "versionNotes": "package-solution.json uses 4-part format X.Y.Z.0; see scripts/release.ps1 for bump logic"
  },
  "activation": {
  ...
  "workflow": {
    "shipOrder": ["preflight", "test", "problems", "bump", "commit", "tag", "push"],
  ...
  "docsInfraPolicy": {
    ...
    "filePatterns": ["*.md", "*.json", "*.yaml", "*.yml", "*.ps1", "docs/**", ".github/**", "apps/**", "contracts/**", "scripts/**", "LICENSE*", "CONTRIBUTING*"],
    "excludeWhenCodePresent": ["*.ts", "*.tsx"],
    "comment": "For docs/infra-only changes: skip npm test (Heft/Jest), keep editor diagnostics (e.g. markdown lint, ESLint)"
  },
  "build": {
    "defaultProfile": "dev",
    "releaseOnlyWhenRequested": true,
    "comment": "Default to dev builds (npm run build uses Heft). Production builds bundle and package .sppkg."
  },
  "versioning": {
    "scheme": "semver",
    "patchEveryShip": true,
    ...
    "minorBranchPrefixes": ["feature/", "app/"],
