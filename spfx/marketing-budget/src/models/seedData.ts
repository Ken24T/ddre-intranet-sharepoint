...
import type { Vendor, Service, Suburb, Schedule, DataExport } from "./types";
...
export function getSeedData(): Partial<DataExport> {
  ...
  return JSON.parse(
    JSON.stringify({ vendors, services, suburbs, schedules }),
  ) as Partial<DataExport>;
}
